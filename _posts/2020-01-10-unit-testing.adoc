...
As seen in the diagram there is one class that acts as the system that should be tested (system under test: SUT). This class uses another class that is responsible for helping some additional need. This class is normally referred to as a dependency.

What Stubbing or Mocking is doing now is the following:

In order to isolate the SUT from any other dependency in order to gain the test locality, the dependency is replaced with a Stub implementation. This object plays the role of the real dependency, but in the test case you control the behavior of the Stub. With that you can directly influence how the SUT behaves even if it has some interaction with another dependency that would otherwise not be controllable directly.
...
This test implementation is another form of exchanging the dependency in the test case. Using a Mocking framework or providing static test classes is more or less the same from a 10'000 feet point of view and support the same goal.
