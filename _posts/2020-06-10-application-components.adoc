...
The `@Extends` annotation replaces all occurences of the `Visit` entity with its new extension  `RockyMountVisit`. As the subclass can still be treated as a `Visit` this also means, all existing logic / screens that is inside the application component will continue to work.
...
----
<1> a input dialog facet is used for requesting certain information before the visit is created
...
@Entity(name = "lancasterpetclinic_TreatmentRoom")
...
[source,java]
----
@Entity(name = "lancasterpetclinic_DefaultTreatmentRoom")
...
.LancasterVisitEdit.java
[source,java]
----
@UiController("petclinic_Visit.edit")
@UiDescriptor("lancaster-visit-edit.xml")
public class LancasterVisitEdit extends VisitEdit {
    ...
    private void initTreatmentRoom(TreatmentRoom treatmentRoom) {
        final LancasterVisit visit = (LancasterVisit) getEditedEntity(); // <2>
...
<2> in order to use the methods of the extension, the Visit entity has to be casted to `LancasterVisit`. As we have extended the entity, we can be sure that CUBA will always use `LancasterVisit` instead of `Visit`. Therefore it is safe to cast here.