...
<2> by adding another form component with the same ID "fieldGroup" the component will be merged with the parent one
...
In this guide we have learned about the different options on how to implement product customizations via various extension mechanisms. Mainly there are the two options Runtime extensions as well as Compile-Time extensions. CUBAs application components fall under the category of Compile-Time extensions, which we used throughout the Petclinic example. They mainly differentiate in their capabilities to adjust the original software. It is a trade-off between adjustment needs and deployment flexibility. Runtime extensions can be changed at deployment / runtime, where compile-time extensions do not have this possibility.

When switching to the concrete example of the Petclinic, we explored what to do in order to make the Petclinic an application component in order to use it in customization projects.
...
The extensions mechanisms CUBA provides for the UI layer allows highly customizable product extensions without the need to duplicate a lot of source-code.

The ability to extend (and replace) entities is quite unique in the Persistence Framework space. It allows the developer to replace the usage of an Entity in the overall application and adding additional attributes to the entity. CUBA Studio additionally takes away the need to manually deal with the special case of altering the existing DB table of the parent entity.


== Further Information

* https://doc.cuba-platform.com/manual-latest/entity_extension.html[CUBA docs: Entity Extensions]
