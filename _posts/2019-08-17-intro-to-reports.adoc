...
As described above, the template contains regular text as well as the placeholders for the different columns in the table like `${pet.name}` or `${pet.identificationNumber}`.
...
The data that is used for the report can be of various sources. Within a CUBA application it is possible to fetch data directly from the entity layer of the application as the most convenient variant. However, the reports application component is not bound to this particular data source. It can be one of the following data sources:
...
Once the data sources are defined and connected through a concept called Bands to particular template placeholders, everything is setup to generate the output document. CUBA stores all of this configuration in a entity `Report`, that holds the configuration of the data sources together with the definition of the templates as well as further configuration information.

image:https://dummyimage.com/600x400/00b5ec/ffffff.png&text=CUBA Report UI configuration[]
