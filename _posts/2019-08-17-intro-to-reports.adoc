...
In this variant, the report configuration itself determines in which screens it should be "available". The remaining code in the controller just declares, that it wants to print a report and the report application component will then figure out which reports are possible in this context.
...
<2> the `EditorPrintFormAction` from the reports application component is used which automatically searches for reports that configured this screen
...
In this introduction to reports and document generation, the building blocks of the CUBA application component `reports` were explained.
A report is the concept that allows to merge a document template with data from data sources to produce an output document.
...
All this configuration is hold in the main entity: `Report`, which can be created through a UI wizard or manually through the configuration UI.
With that differentiation it supports users with different levels of technical capabilities.
...
In this guide two reports were created. The first report generated an excel sheet containing the visits of the last month. In this case external parameters were leverages to
