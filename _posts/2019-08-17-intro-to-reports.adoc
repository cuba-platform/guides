...
For defining the query conditions two conditions will be added for the attribute `visitDate`, because we want to apply two constrains on the same attribute: the `visitDate` should match a value that is within an range of two dates: "Visit Date Range Start" and "Visit Date Range End". The only difference between the two conditions is that for the first condition the "operation" is `greater or equal than: >=` while the second condition has the operation `smaller than: <`.
...
The next step in the wizard is to define the attributes of the base entity (Visit) and its associated entities that should be part of the report. The selection will influence on the one hand the `SELECT` part of the JPQL statement that is used to fetch the data and the other hand the initial version of the columns within the templates, that is generated for this report when finishing the report wizard.

For the Recent Visit Report, the following attribute should be part of the report:
...
After selecting the attributes through the UI wizard, the output template file type can be configured.

With that the report wizard is finished and the report configuration is shown for further adjustments.

=== External Parameters

A Report configuration can declare external Parameters that need to be provided by the outside world. Those parameters are mainly used for the data fetching parts. In the JPQL / SQL statements as well as the groovy based data bands, those parameters are passed in to use them to dynamically inject the values into the query.
...
public class VisitBrowse extends StandardLookup<Visit> {
    ...
    protected void onVisitsTableLastMonthReport(Action.ActionPerformedEvent event) {
        MonthYearValue lastMonth = MonthYearValue.fromDate(today()).minusMonths(1); // <1>

        reportGuiManager.printReport( // <2>
                loadReportByCode("visits-by-period"), // <3>
                ParamsMap.of( // <4>
                        "visitDateRangeStart", lastMonth.getStart(),
                        "visitDateRangeEnd", lastMonth.getEnd()
                        )
        );
    }
...
<3> a parameter map is passed in containing the start and end date range parameter of the report through the `MonthYearValue` object
