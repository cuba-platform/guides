...
public class PetEditTest {
    ...
    @Test
    public void identificationNumberIsCorrectlyBoundInTheInputField() {
...
public class OwnerBrowseTest {
    ...
    @Test
    public void when_ownerListIsDisplayed_thenOwnersAreShownInTheTable() {
...
The remaining part left of the manual data configuration is the definition of the mock call and the expected result.
For that we use the Mockito API: `when(...).thenReturn(...)`. In this case, we need to be a little more precise, because
we don't want to mock all calls to `DataService.loadList(...)` but instead only the one where the load of the `Owner`
entity happens.

The helper method `ownerListIsLoaded` defines that situation. `Mockito.argThat` is the API that lets the test define a
Predicate lambda expression that determines if the parameters of a call to the method (`loadList` in this case) are
matching against the call that we want to mock. Basically it acts as a filter, to pinpoint the call that we are searching
for.

The Lambda gets passed in the original parameter of the method: `LoadContext`.
`loadContext.getEntityMetaClass().equals("petclinic_Owner")` is our filter criteria to verify that the entity class is the
`Owner` entity.
...
defines the combination of the mock with the return values. In case the owner list is loaded, then Ash and Misty
should be returned.
...
public class OwnerBrowseTest {
    // ...
    @Test
    public void when_ownerListIsDisplayed_then_ownersAreShownInTheTable() {
...
The second test case builds on the fundamentals of the first one that we just explored. In this case the goal is to test
the correct interaction between two screens. The following steps are performed by the test:
...
public class OwnerBrowseTest {
    // ...
    @Test
    public void given_ownerIsSelected_when_editIsPerformed_then_ownerEditorIsOpened() {
...
In this test case we used additional parts of the CUBA UI API to interact with the application e.g. via
`ownersTable.setSelected(...)`. Also we used it to retrieve some information that is necessary in the test, like the
search for an opened screen.
...
the "Disease Warning Mailing" functionality. For the middleware logic of the functionality, we have already
created a middleware integration test as described in the https://www.cuba-platform.com/guides/integration-testing-middleware#disease_warning_mailing[CUBA Guide: Middleware Integration Testing].

In the middleware integration test, we covered everything from the Service Interface and below, but left out the UI part.
In this guide, we will now do the opposite. We will create an integration test, that interacts with the UI, but mocks
out the behavior of the Service Interface. This way we can control the behavior of the dependency (`DiseaseWarningMailingService`)
and ensure that the UI code interacts with it according to our expectations.
...
class CreateDiseaseWarningMailingTest {
    // ...
    @Test
    public void when_createDiseaseWarningMailingFormIsFilled_thenMailingIsSendOut() {

        PetBrowse petBrowse = openScreen(PetBrowse.class);

        Table<Pet> petTable = petTable(petBrowse);

        petTable
                .getAction("createDiseaseWarningMailing")
                .actionPerform(createDiseaseWarningMailingBtn(petBrowse));

        CreateDiseaseWarningMailing createDiseaseWarningMailing = findOpenScreen(CreateDiseaseWarningMailing.class);

        cityField(createDiseaseWarningMailing)
                .setValue("Alabastia");

        diseaseField(createDiseaseWarningMailing)
                .setValue("Fever");

        PetType electricType = data.electricType();
        petTypeField(createDiseaseWarningMailing)
                .setValue(electricType);

        createDiseaseWarningMailing
                .getWindow()
                .getAction("createDiseaseWarningMailing")
                .actionPerform(createMailingBtn(createDiseaseWarningMailing));


        verify(diseaseWarningMailingService, times(1))
                .warnAboutDisease(electricType, "Fever", "Alabastia");
    }
}
----
