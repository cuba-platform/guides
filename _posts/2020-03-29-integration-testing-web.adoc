...
These are limitations with a web integration test. To overcome them, you need a black-box end-to-end test case. CUBA has a dedicated library called https://www.cuba-platform.com/marketplace/masquerade/[Masquerade] that makes creating those test cases easier. It is based on Selenium and therefore performs real browser based tests that interact with the application from the UI to the database.
