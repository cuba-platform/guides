...
TIP: the `query` methods also support different shortcut variants for defining the JPQL query like `query("e.owner.city = ?1", ownerCity)`. More information on the shortcuts can be found in the https://doc.cuba-platform.com/manual-{cuba_version}/dataManager.html#dm_query[docs].
