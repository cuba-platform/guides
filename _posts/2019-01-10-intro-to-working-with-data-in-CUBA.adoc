...
这个指南将向你介绍一些 CUBA 的API，了解了这些API后你可以使用 DataManager API以编程方式与数据库进行交互。


== 将要构建什么

这个指南对 https://github.com/cuba-platform/cuba-petclinic[CUBA 宠物医院]示例进行了改进，以便演示出使用DataManager API 进行编程式数据访问的各种场景。特别是我们会涉及到以下场景：
...
通常，以编程方式与数据库进行交互是必要的。在基于CUBA的应用程序中，由于可以以声明的方式对UI进行数据绑定，所以编程式数据访问不会像其它框架那么多，但仍有很多情况我们必须在业务逻辑代码中读写数据库。

CUBA 提供的主要数据交互点是 https://doc.cuba-platform.com/manual-{cuba_version}/dataManager.html[DataManager] API。这个API允许 CUBA 应用程序开发人员通过对实体的一般抽象来加载和存储数据库中的数据。????
...
DataManager 有多种方式从数据库加载数据。在这个指南中，介绍两个主要方式：基于查询加载单条数据和多条数据。
...
存储实体实例的过程包含几个步骤，如安全检查、执行实体监听器等。 commit 方法的返回值是存储的实例，这个实例包含了所有在存储阶段添加的属性。????
