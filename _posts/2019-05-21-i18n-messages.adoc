...
After ensuring the target module is available in the application, the package where the original translations files exists, have to be created in the application as well. In this directory the additional / replacing message files are placed. CUBA will pick up the file and display the correct translations for the screen.
...
WARNING: Not all Buttons are translated in this example. This is due to the fact, that e.g. the filter component with its buttons and captions like `Search`, `Add search condition` etc. are not located in this package. In case all captions of the Screen should be translated, it is required to look into the sources of the platform screen to identify where a particular translation is located.
