...
The second way of interacting with the Localization mechanism is not through the declarative XML definitions, but imperatively through Java code. In the Petclinic example, the "regular checkup" button should render the following two dynamic messages programmatically:
...
Messages can be received via the `MessageBundle` Interface. `MessageBundle` is a new shorthand mechanism introduced in CUBA 7.0, that fetches a particular message from the message pack of the calling Java class.

Alternatively there is the `Messages` Interface, which is a little more flexible and contains additional APIs, but requires to always define the package for every message lookup.
...
Since CUBA comes with certain UI components and with multiple UI screens that are available out of the box, this existing
functionality needs also to be localized in order to have a fully translated application.
...
The first and most easy solution is to use existing Language packs that are created by the community. There are currently
twelve Translations available including Spanisch, Russian, French, Chinese, German and a lot of more. The translation cover around half of the worlds population.
It is worth mentioning though, that since the Translations are provided by the community, the accuracy and the catch up
with the latest changes in the platform differ and are provided as best effort.
...
In case the target language is not available through the community or some Translations are missing / not accurate, it is also
possible to provide those translations directly within the application.

CUBA itself provides translations for the screens directly in the source code of the platform as regular `*.properties` files.
The User browse screen for example contains the https://github.com/cuba-platform/cuba/blob/master/modules/gui/src/com/haulmont/cuba/gui/app/security/user/browse/messages.properties[messages.properties] file in the package `com.haulmont.cuba.gui.app.security.user.browse`,
which contains the translations for the screen.

In order to provide / override translations for this screen, the same package (and module) has to be created in the application.
In this directory the additional / replacing message files are placed. CUBA will pick up the file and display the correct
translations for the screen.
