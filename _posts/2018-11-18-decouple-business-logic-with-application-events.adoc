...
There are several Events to subscribe that deal with the lifecycle of a controller like `InitEvent`, `BeforeCloseEvent`, `PreCommitEvent` and so on. The data part of the controllers also offer a list of Events like `ItemChangeEvent`, `CollectionChangeEvent`. Furthermore the UI components itself send events for changes of their states like `EnterPressEvent`, `TextChangeEvent` and so on.
