...
TODO: The form in the employee edit screen just declares a field for the `employeeRecord` attribute.
...
* In the https://github.com/cuba-guides/{project_id}/blob/master/modules/global/src/com/haulmont/sample/petclinic/web/employee/employee/EmployeeSingleEditorEdit.java[EmployeeSingleEditorEdit.java] controller we override the `initNewItem()` method. It will create a `EmployeeRecord` instance and link it to the new `Employee` instance when the latter is just created:
+
...
Finally, let's handle the situation when a user clicks *Create* and then wants to close the editor without changing anything. This user will be asked for saving changes, as the `employeeRecordDc` data container already contains an empty instance and `isModified()` method of the `StandardEditor` will always return `true`. To prevent the appearance of confirmation dialog, we should make `isModified()` consider changes only in the master data container:
...
In this data modelling guide the composition relationship was described. Compositions are valuable if a master-detail relationship between two entiities should be modelled, where the detail entities can only exists if the corresponding master entity exists.
