...
This guide enhances the https://github.com/cuba-platform/cuba-petclinic[CUBA petclinic] example to show various example of a composition relationship between entities. In particular, the following use cases will be covered:
...
* A user can pick a `address` and open its editor. When *OK* is clicked in the `Address` editor, the updated instance of the `Address` is not yet saved to the database, but to the `addressesDc` data container of the `Owner` editor.
...
* When a user clicks *OK* in the `Owner` edit screen, the updated `Owner` instance together with all the updated `Address` instances is submitted to the `dataManager.commit()` method on the middleware and saved to the database within a single transaction.

=== One-to-Many: Two Levels of Nesting

Composition can also be multiple levels. The next example shows a Composition of two levels. In the Petclinic there is need to store health records of the pets. The `Pet` -> `HealthRecord` relationship is another composition. However, also the health record has another composition, since there can be multiple Attachments created for a given health record.

image::data-model-composition/composition_domain_model_two_levels.png[align="center"]

The `Pet` entity contains the `healthRecords` attribute – a collection of the `HealthRecord` instances. In order for all three entities to become a single composition and be edited together, the following should be done in addition to the steps described above:
...
The employees of the pet clinic have to be managed (like Vets, Nurses etc.). Therefore a `Employee` entity is defined. For each `Employee` instance there is an associated `EmployeeRecord` entity, which stores information about the work related information of this employee.
...
The definition of the `employeeRecord` property in the `employee-edit.xml` is defined as a `pickerField` component which contains the following actions:
...
The employee edit screen contains the PickerField component with two actions: `OpenAction` and `ClearAction`:

image::data-model-composition/composition-one-to-one-editor-open-action.png[align="center",width="600"]

When the open action is invoked, a new instance of `EmployeeRecord` is created and its edit screen is shown. When *OK* is clicked in the employee record editor, the employee record instance is not saved to the database, but to the `employeeRecordDc` data container of the employee edit screen.
...
When a user clicks *OK* in the employee edit screen, the updated `Employee` instance together with the `EmployeeRecord` instance is submitted to the `DataManager.commit()` method on the Middleware and saved to the database within a single transaction.
...
It is often convenient to edit the One-to-One composition in a single editor. Therefore the above mentioned example will be implemented so that the `EmployeeRecord` can be edited within the `Employee` editor screen.

* https://github.com/cuba-guides/{project_id}/blob/master/modules/global/src/com/haulmont/sample/petclinic/web/employee/employee/employee-single-editor-edit.xml[employee-single-editor-edit.xml] descriptor contains the main `employeeDc` and the nested `employeeRecordDc` data containers:
+
--
[source, xml]
----
...
Fields for editing both entities are grouped into one `form`, where some fields are bound to the nested data container:
...
Finally, the situation needs to be handled when a user clicks *Create* and then wants to close the editor without changing anything. This user will be asked for saving changes, as the `employeeRecordDc` data container already contains an empty instance. To prevent the appearance of confirmation dialog, `isModified()` consider changes only in the master data container:

TODO: what is the CUBA 7 way of doing isModified() - just isEntityModified()?
...
Composition structures can exists in 1:1 and 1:N form. Furthermore composition can be created across multiple levels. The corresponding UI screens treat Composition relationships in a special way. Detail entities are only stored during the creation process of the master entity at the point in time when the master entity is stored.