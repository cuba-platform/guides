...
Another similar example is to find the Pet via its identification number. This could be achieved as well by the search API, but in this case, let's take a look at using custom service to provide this capability. The `PetService` that is part of the demo application has a method: `findById(String petIdentificationNumber)` which returns a Pet instance. It is possible to expose that service directly through the generic REST API, so that the method can be invoked via the API.
...
----
<1> the method `findById` is marked as being part of the generic REST API for authenticated users

This file also needs to be attached to the configuration via the `web-app.properties` like this:

```
cuba.rest.servicesConfig=+com/haulmont/sample/petclinic/rest-services.xml
```

TIP: CUBA Studio allows to automatically generate the file and the `web-app.properties` binding by right clicking the "REST API" section in the CUBA menu and select "Create REST Services Configuration".
...
By exposing service methods as an API it is possible to expose business logic via APIs without the need to develop the HTTP interactions on the server side. This allows to create an abstraction on top of the direct entity access in order to ensure data integrity, further validation etc.
...
Developing a custom API is time-consuming in particular when a big API surface is needed. On the other side it is oftentimes required in case strict API backwards compatibility is a must have, because you do not control the API clients.
