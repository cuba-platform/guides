...
After seing how to leverage the generic REST API of CUBA, lets look on how to create those APIs via custom REST controllers as shown in the beginning via Spring MVC. Let's go through the four scenarios and see how to tackle one-by-one.
...
The first RESTful HTTP API that is requested by `PetTreat` is to load Pet and Visit Information for display purposes. For the `fetchPet` controller, we already have seen an example above. For the Visit fetching, the difference is that a dedicated response object is used:
...
----
<1> the Visit entity is loaded via a dedicated `VisitService`, that interacts with the database
<2> the Visit is transformed into `FetchVisitApiResponse` for exactly defining the API response

Here are two interesting points to elaborate a little further. First is the fact, that the controller is very small in its functionality scope. Although it would be possible to perform the data fetching directly against the database and potentially also do data transformation, those responsibilities are put into different classes. It mostly is a good idea to keep the controller layer thin and only perform its the main responsibility: doing HTTP interactions and delegate business logic to other classes.
...
The next operation is to create a visit for a Pet. This endpoint also is located in the `VisitController` as another endpoint:
...
In this example it is once again showing the application of the decoupling of the inner workings of the Petclinic with the outside world. The dedicated API objects `CreateVisitApiRequest` and `CreateVisitApiResponse` represent the contract with the API client. Those classes are only used in the controller layer. Internally when interacting with the `VisitService` the request object is converted to another DTO called `VisitCreation`, that is located in the global module of CUBA. This enforces proper layering between the REST API and the internal service layer.

The `@Valid` annotation on the request parameter indicates to Spring, that the request object needs to be validated via bean validation. In case the JSON request contains invalid data, Spring will return a proper error response with an error message of the bean validation result.


==== Starting and Stopping a Visit Treatment

The last business requirement is to be able to start and finish Visit treatments via `PetTreat`. The API is similar to the create visit API, but it shows one interesting difference: API as an abstraction. Up until now the API more or less mirrored the domain model of the Petclinic. Oftentimes this is enough, but sometimes it is possible to use the API layer as a way to simplify the internal domain model or hide certain details.
...
Internally the API performs changes on the `Visit` entity. In particular the `visitStatus` attribute is effected. Still, on the API the model and concept to interact with are `Treatments`. Here is an example of the HTTP request to finish a treatment:
...
With closing off those examples, we finished the complete Petclinic example or the REST API. With those two different approaches in mind, let's look into the pros and cons of the different approaches.
...
The functionality of loading entites in various ways with proper pagination, search capabilities and a mechanism to define queries in the request give huge flexibility for the client of the API without touching the server side are highly beneficial. Through referencing different views, it is also possible within the client to control the amount of data that are returned. In the example we saw above where the search operation was invoked with the view `pet-with-owner-and-type` not only the pet, but also the reference to the owner and its type are returned in the JSON.

With the built-in security mechanism it is possible to transparently on the server side control what data the API client is able to retrieve, just as it is possible in the UI.

==== Custom API

The custom Petclinic API is leveraging Spring MVCs mechanisms to control the HTTP interaction in very precise way. It is possible to define the input data as well as the output data. HTTP response codes can be used to describe particular situations in the HTTP lifecycle like error scenarios.

By defining the input and ouput data via dedicated request and response objects, it is not only possible to control which data is returned, but furthermore it allows to define higher level API abstractions on top of the entity model. Not every database entity should be accessible via APIs The decoupling allows to free yourself from changing your internal entity model without changing your API client.

The generic API has some mechanisms in place to overcome those shortcomings. E.g. it is possible to define https://doc.cuba-platform.com/restapi-latest/#rest_api_v2_data_model_versioning[model-transformations] to have backwards compatibility when an entity attribute is changed, but those are limited in nature compared to controlling the input & output data in java classes. Also, the possibility to invoke CUBA services directly provides a way to decouple the entity model from the API to a certain degree.

Developing a custom API is oftentimes time-consuming depending on the API surface. On the other side it is oftentimes required in case API compatability is a must have.

CUBA together with Spring provides mechanisms to fulfil both ends of the spectrum. Depending on the use-case and requirement either a custom API or the usage of the generic API can be used. It is also possible to use both approaches hand-in-hand. E.g. the search capabilities can be used via the generic API, while the write operations are performed through custom API controllers.


== Summary

In this guide we have learned about the basics of the architectural style REST and its concrete application within HTTP. After going over the basics like Resources, Verbs as well as Requests and Responses. After that we shifted our focus to take a look on how to create HTTP endpoints in Spring.
