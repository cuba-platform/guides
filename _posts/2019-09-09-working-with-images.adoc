...
TIP:  使用专门的工厂类来定义创建组件的方式只是一个对实现进行结构化的可能选项。将组件的创建提取到专门的类有几点好处，比如可以做到封装、关注点分离及在其它界面重用。 +
 +
但是也可以将组件的创建逻辑直接写在控制器中。这种方式可以在一个关于如何在UI层结构化应用程序逻辑的示例中看到。关于这个主题的详细信息可以参考这个指南：https://www.cuba-platform.cn/guides/create-business-logic-in-cuba[在 CUBA 中创建应用程序逻辑]。
...
<3> 已经持久化的文件的 `FileDescriptor` 实例被分配给X光 images  `M:N` 关系，以显示在表格中 ???
...
<1> GroupBox specific styles will be applied to the wrapper component ???? 测试
...
CUBA 应用程序中的图片渲染和自定义元素组件可以有效地提高用户体验。主要的构建块(building block )是 `FileDescriptor` 抽象，所以的UI组件都可以与 `FileDescriptor` 平滑地集成。 ???? 改进
