...
:page-excerpt:  这个指南演示为实体实例设置初始值的几种方式
...
这个指南展示了关于初始化实体值的几种方式。一种方式是在实体上使用 `@PostConstruct` 注解直接进行属性值的分配。
...
此外，演示了两种不同的依赖属性的初始化，as their use case is oftentimes quite similar to the normal attribute initialization 。可以通过表单字段的 change  事件初始化属性，或者通过数据上下文事件在实体保存前初始化属性。 ????
