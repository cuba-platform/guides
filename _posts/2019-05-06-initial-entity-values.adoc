...
public class VisitNumberGenerator {
    ...
    public String generateVisitNumber(Visit entity) {
        ...
        long nextVisitNumber = uniqueNumbersService.getNextNumber(String.format("VISIT_%d_%d", visitYear, visitType)); // <1>
...
Furthermore two different dependent attribute initializations were shown, as there use-case is oftentimes quite similar to the normal attribute initialization. It is possible to either initialize a dependent attribute directly on change of a Form field or before storing the entity with the help of the data context lifecycle events.
