...
The first step is to upload the X-Ray image to a particular `Visit` instance. For this the following `<upload />` component will be placed in the `<buttonsPanel />` component of the `xRayImagesTable`. Compared to the first use case, this time the persistence part will be handled manually.
...
The corresponding `VisitEdit` controller subscribes to the `FileUploadSucceedEvent` of the `upload` component and adds the file to the `M:N` association of the visit instance.
...
With that subscription code in place, the X-Ray image upload is implemented and can be used to fulfil the next requirement: preview of the uploaded image.
...
The last functionality that is part of the X-Ray Image preview functionality is that it should be possible to download the image files.

For this, an additional button is placed on top of the `xRayImagesTable`. The controller code, uses the `ExportDisplay` bean from CUBA in order to trigger the download of a FileDescriptor in the browser.
...
The `exportDisplay` bean has multiple options to download a file. The one that is used here takes a `FileDescriptor` instance. The constant `ExportFormat.OCTET_STREAM` indicates the browser that it should force the file to be downloaded and not try to render it within the browser.

== Summary

Image rendering within CUBA applications as well as custom composition of components will enrich the CRUD experience for users immediately. The main building block is the `FileDescriptor` abstraction that all UI components integrate seamingless with.
