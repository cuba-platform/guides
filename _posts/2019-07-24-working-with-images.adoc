...
The most interesting part is the `xrayImageWrapperLayout` component, which acts as a placeholder that will contain the image later. Currently it contains no child components. It will be filled at the time a selection is made for the `xRayImagesTable`.
...
<3> the logic to create the X-Ray image preview is delegeted to the `XrayPreviewComponentFactory` class
