...
The `exportDisplay` bean has multiple options to download a file. The one that is used here takes a `FileDescriptor` instance. The constant `ExportFormat.OCTET_STREAM` indicates the browser that it should force the file to be downloaded and not try to render it in another tab e.g.
