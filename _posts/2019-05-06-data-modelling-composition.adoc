...
本指南对 https://github.com/cuba-platform/cuba-petclinic[CUBA 宠物诊所] 进行了增强，以演示实体间组合关系的各种示例。特别是会覆盖以下用例：
...
CUBA 平台支持两种类型的实体间关系： `关联(ASSOCIATION)` 和 `组合(COMPOSITION)` 。 关联是对象间可以独立存在的关系。而组合，换句话说就是用于 “主-从”关系，这种关系中明细实例不能脱离主实体而存在。
...
典型情况下，组合关系的实体总是一起被编辑，因为这样很自然、合理。用户打开  `Owner` 编辑界面看到 `addresses` 列表，
