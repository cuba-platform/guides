...
开发 CUBA 应用程序时的首要问题之一是： _我的业务逻辑应该放到哪？_  本指南将介绍几种选项和各自的优缺点。

== 将要构建的内容


本指南对  https://github.com/cuba-platform/cuba-petclinic[CUBA 宠物诊所] 示例进行增强，以演示在 CUBA 应用程序中把业务逻辑写在哪。在本指南中展示了一个计算折扣的业务逻辑， 折扣主要根据宠物在诊所中的到访次数来计算的。
...
另外，有几种高级技术，比如 https://doc.cuba-platform.com/manual-6.10/jmx_beans.html[JMX Beans] 或 https://doc.cuba-platform.com/manual-6.10/entity_listeners.html[实体监听器] 也允许创建在特定场景下要执行的业务逻辑。对于特定场景，在什么位置放置业务逻辑应视具体情况而定。但是前面提到的的几个主要位置：界面控制器、客户端Spring Bean及 中间件服务是一个不错的起点。
