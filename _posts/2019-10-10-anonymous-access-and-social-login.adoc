...
Applications such as an online store provide users the ability
to view a list of available products, read their description, or compare
with each other without logging in. But in order to make a purchase,
the user must register, and often he can do this using social network account.

== What Will be Built

This guide enhances the CUBA Petclinic example to demonstrate how to enable
application public access and allow users to login and register via social services.
...
in clinic and animals that can be brought for treatment without logging in
...
Click the right button in menu to proceed to login screen and enter the system as administrator.
...
In this section, we'll implement login using a modal dialog.
...
dialog - install custom login handler via `@Install` annotation:
...
with creating new blank screen. To avoid re-implementing login process logic
we’ll just make `LoginDialog` extend `LoginScreen`:
...
Dialog layout is just a login form copied from default screen:
...
Fix dialog width and perform login on button click:
...
Now we can open our new dialog:
...
The last step is to check whether login performed successfully and close dialog:
...
First of all to be able to authenticate your app you should register it
...
2. The service sends a response with a temporary code
3. Application sends a request with client id, client secret and the given code to the service
4. The service sends a response with an access token
...
The easiest way to add social buttons into the Login Dialog
is to use few `LinkButton` components placed into horizontal box layout:
...
We’re using custom style name to make buttons bigger. Open `hover-ext.scss`
...
Not all services support `localhost` as application host. The easiest solution
is to add a host alias to the operating system hosts file
...
Now we can subscribe to social button clicks and redirect a user to a social service login page:
...
To handle a response with the temporary code we will use Vaadin Request Handlers mechanism -
...
so it should be implemented as a bean. Request handlers should be added
...
* Extract auth code and fetch user data via `SocialLoginService`
* Create `Credentials` instance based on user data
* Trigger login and redirect the user back to app

We’re using injection via setters approach here to obtain required components.
The handler also implements `InitializingBean` interface to finish initialization
when all beans are injected. Firstly, we use the `UIAccessor` instance to lock
the UI until the login request is processed:
...
Get back to `LoginDialog` and add our handler:
...
When the auth code is available, we can use it to get an access token:
...
Form a new URL and perform a request using `HttpClient` library:
...
The final step is to parse user data from the response into `SocialUserData` POJO:
...
To implement custom login you should create your own `Credentials` and `LoginProvider`
that supports such credentials.
...
It’s designed to pass general user profile info and specify a service that is used to login.
Use this method to form new credentials in callback handler:
...
When we have social credentials login process can be triggered:
...
Default implementation of `Connection` uses all available `LoginProviders`
to get new authentication info. `LoginProviders` mechanism enables you
to use ordered Spring beans to authenticate a user for different types
of credentials. We will use this extension point to login:
...
To avoid re-implementation of the authentication procedure, we will extend
built-in `ExternalUserLoginProvider` and pass new `ExternalUserCredentials`
based on the available info to the super method:
...
The final step is to register a new user considering social profile info.
First of all, create a new entity `SocialUser` extending `User`
...
Create a new service `SocialRegistrationService`:
...
public class SocialRegistrationServiceBean implements SocialRegistrationService {
    ...
    public User findOrRegisterUser(String socialServiceId, String login, String name,
        ...
        SocialRegistrationConfig config = configuration
                .getConfig(SocialRegistrationConfig.class);

        EntityManager em = persistence.getEntityManager();
        Group defaultGroup = em.find(Group.class, config.getDefaultGroupId(),
                View.MINIMAL);
