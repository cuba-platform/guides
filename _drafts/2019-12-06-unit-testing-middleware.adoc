...
First you will understand how testing in general works in a CUBA application. Afterwards this guide will concentrate on the area of integration testing on the middleware layer.
...
send out to a target Owner this is a little bit of a problematic area to verify. How can the test case assure that an Email was actually send out, because the test case itself is not directly involved in this interaction. Generally there are the following verification options:

1. check for a received Email in a valid Email Account programmatically
...
Those three different types of verification can be categorized either into a State verification or into a Behavior verification. A behavior verification looks at the two collaborators (can be classes, modules or even systems) and based on the fact that a particular interaction took place derives the conclusion that the verification is correct.
...
TIP: Try to avoid double-test functionality provided by a Framework because this is already trusted & tested code _in integration tests_.
